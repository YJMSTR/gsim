...
| 16 | 6.43 s (dense, registered v457; 6.08–6.26 s pinned at registration+0 on 2026-08-24; compaction measured +1.5% on this tier — T32-recipe-only) | 11.15 s | 1.73× |
| 32 | **4.83 s** (`GSIM_MT_DENSE_VCONTRACT_COMPACT=1`, registered xiangshan-t32-compact-v2, 2026-08-27 clean 5-run; legacy champion 5.33–5.37 s on the same date) | 9.73–9.84 s | ~2.03× |
...
| gsim-mt, defaults (T32: L2+chunk) | 8m31s (champion seed replay) | **1m18s** (507 TUs) | **~9m49s** replay | **4.8 GB** |
| gsim-mt, + `GSIM_SHORT_NAMES=1` (T32) | 8m36s | 1m46s (89 TUs; emu 74 MB) | ~10m22s | 1.1 GB (**+4.5% C50000**) |
...
- **Default under the dense recipe** (`--mt-helper-mode=mt-level-dispatch` + `GSIM_MT_DENSE_EXECUTOR_CODEGEN=1`): `GSIM_MT_DENSE_ONLY_CODEGEN=2` + `GSIM_EMIT_RESET_CHUNK=4096` (perf-neutral, verified). `GSIM_SHORT_NAMES` was flipped on and **reverted after a T32 5-pair measured +4.5%** (shorter identifiers shift znver4 codegen alignment; T16 +1.4% same direction) — it stays opt-in for size-constrained builds, at a documented ~4.5% T32 C50000 cost. Export any `=0` for the legacy full emission byte-identically.
